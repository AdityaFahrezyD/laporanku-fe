    ...
    <main className="grid min-h-dvh lg:grid-cols-2">
      <section className="relative hidden flex-col justify-between overflow-hidden bg-primary p-14 text-white lg:flex">
        <a href="/" className="flex items-center gap-3 text-xl font-semibold">
          <Icon name="leaf" className="size-8" />
          LaporanKu.
        </a>
        <div className="relative z-10 max-w-md">
          <p className="mb-5 text-xs font-semibold uppercase tracking-[0.2em] text-[#E7B780]">
            Pembukuan bersama
          </p>
          ...
          <p className="mt-6 leading-relaxed text-white/65">
            Satu tempat untuk melihat dan mengelola perjalanan keuangan bersama.
          </p>
        </div>
        <p className="text-xs text-white/50">LaporanKu · Ruang administrator</p>
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -bottom-48 -right-48 size-[36rem] rounded-full border-[70px] border-white/5"
        />
      </section>
      <section className="flex items-center justify-center px-5 py-12 sm:px-10">
        <div className="w-full max-w-sm">
          ...
          {checking ? (
            ...
            <p role="status">Membuka dashboard…</p>
          ) : user ? (
            <div className="space-y-4">
              <Alert variant="warning" title="Akun ini bukan administrator">
                Gunakan akun admin untuk mengakses halaman ini.
              </Alert>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {(error || sessionError) && (
                <Alert variant="error" title="Belum bisa masuk">
              ...
              <div>
                ...
                <input
                  ...
                  aria-describedby={
                    error?.errors?.email ? "email-error" : undefined
                  }
                  className="mt-2 w-full rounded-xl border border-primary/20 bg-white px-4 py-3 text-sm disabled:opacity-60"
              ...
              <div>
                ...
                <div className="relative mt-2">
                  <input
                    ...
                    required
                    disabled={submitting}
                    aria-invalid={Boolean(error?.errors?.password)}
                    aria-describedby={
                      error?.errors?.password ? "password-error" : undefined
                    }
                    className="w-full rounded-xl border border-primary/20 bg-white py-3 pl-4 pr-24 text-sm disabled:opacity-60"
                  ...
                  <button
                    ...
                    className="absolute inset-y-0 right-3 text-xs font-medium text-secondary"
                  >
